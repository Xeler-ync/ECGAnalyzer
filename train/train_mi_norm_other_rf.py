...
from utils._train import run_training_multiclass

DEFAULT_RF_CONFIG_PATH = "./config/rf_mi_norm_other.json"
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Random Forest for 3-class: NORM vs MI vs OTHERS"
    )
    parser.add_argument(
        "--mi-dir",
        type=str,
        default="./results/MI",
        help="Directory containing MI data (positive class)",
    )
    parser.add_argument(
        "--norm-dir",
        type=str,
    ...
    parser.add_argument(
        "--others-dir",
        type=str,
        default="./results/OTHERS_exclude_MI_superclass_pure_NORM_code",
        help="Directory containing OTHERS data (label 2)",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
        default="./results/model/mi_norm_other_rf",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="If set, only keep samples with is_pure_norm == 1 in NORM directory",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    # 调用三分类训练
    run_training_multiclass(
        pos_dir=args.norm_dir,
        neg_dir=args.mi_dir,
