...
# ========== 标签定义 ==========
NEGATIVE_LABEL = "NORM"
FOCUS_CLASS_NAME = "MI"          # 目标大类：心肌梗死 (Myocardial Infarction)
OTHERS_LABEL = "OTHERS"

# 根据 scp_statements.csv 中 diagnostic_class == "MI" 的所有代码
# 来源：题目给出的 CSV 片段（包括典型 MI 代码及 subendocardial injury 等）
MI_CODE_SET = {
    "IMI", "ASMI", "ILMI", "AMI", "ALMI", "LMI", "IPLMI", "IPMI", "PMI",
    "INJAS", "INJAL", "INJIN", "INJLA", "INJIL"
}

CLASS_TO_ID = {NEGATIVE_LABEL: 0, FOCUS_CLASS_NAME: 1, OTHERS_LABEL: 2}
TARGET_NAMES = [NEGATIVE_LABEL, FOCUS_CLASS_NAME, OTHERS_LABEL]
...
DEFAULT_OUT_DIR = Path("results/random_forest_mi_vs_norm_vs_others_fixed64_qc")
...
def is_pure_norm(scp_codes: dict, threshold: int = TARGET_THRESHOLD) -> bool:
    """仅当 NORM 得分 ≥ threshold 且所有其他代码得分 < threshold 时返回 True"""
...
def get_multiclass_label_mi_norm_others(scp_codes: dict, threshold: int = TARGET_THRESHOLD):
    """
    多分类标签分配：
    - 0 -> pure NORM
    - 1 -> MI（优先级最高，包含任意 MI_CODE_SET 中的代码且得分 ≥ threshold）
    - 2 -> OTHERS（所有其他至少有一个正得分的非纯 NORM 记录）
    """
    if not isinstance(scp_codes, dict):
        return None, None

    # 检查是否为 MI（任一 MI 代码得分达标）
    is_mi = any(scp_codes.get(code, 0) >= threshold for code in MI_CODE_SET)
    is_norm = is_pure_norm(scp_codes, threshold)

    if is_mi:
        return CLASS_TO_ID[FOCUS_CLASS_NAME], FOCUS_CLASS_NAME
    if is_norm:
        return CLASS_TO_ID[NEGATIVE_LABEL], NEGATIVE_LABEL

    # 其余记录只要至少有一个正得分（非 NORM）就归为 OTHERS
...
def build_dataset(max_records: int | None = None, normalize_mode: str = "none", save_qc_plots: int = 0):
    ...
    for signal_index in tqdm(range(total_records), desc="Building RF dataset"):
        try:
            ...
            label, label_name = get_multiclass_label_mi_norm_others(scp_codes, threshold=TARGET_THRESHOLD)
            if label is None:
            ...
            metadata_rows.append(
                {
                    ...
                    "mi_scores": {code: scp_codes.get(code, 0) for code in MI_CODE_SET},
...
def save_processed_dataset(out_dir: Path, X_3d: np.ndarray, X_flat: np.ndarray, y: np.ndarray, meta: pd.DataFrame, skipped_df: pd.DataFrame, normalize_mode: str):
    ...
    summary = {
        ...
        "label_definition": {
            ...
            FOCUS_CLASS_NAME: f"any MI code from {sorted(MI_CODE_SET)} with score >= threshold (priority if present)",
...
def main():
    parser = argparse.ArgumentParser(description="Random Forest for MI vs NORM vs OTHERS with fixed64 + QC")
    ...
    print("=" * 80)
    print("Random Forest for MI vs NORM vs OTHERS classification (fixed64 + QC)")
    print("=" * 80)
    print(f"Classes            : {TARGET_NAMES}")
    print(f"NORM rule          : pure {NEGATIVE_LABEL} >= {TARGET_THRESHOLD}")
    print(f"MI rule            : any MI code (from {sorted(MI_CODE_SET)}) >= {TARGET_THRESHOLD} (priority if present)")
    ...
    print(f"MI samples          : {int((y == CLASS_TO_ID[FOCUS_CLASS_NAME]).sum())}")
