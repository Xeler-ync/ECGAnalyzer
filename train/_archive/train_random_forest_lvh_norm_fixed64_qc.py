...
DEFAULT_OUT_DIR = Path("results/random_forest_lvh_vs_norm_fixed64_qc")
...
def get_binary_label_lvh_norm(scp_codes: dict, threshold: int = TARGET_THRESHOLD):
    if not isinstance(scp_codes, dict):
        return None
    is_lvh = scp_codes.get(POSITIVE_LABEL, 0) >= threshold
...
def robust_outlier_filter(beats_2d: np.ndarray, mad_k: float = 3.5) -> tuple[np.ndarray, np.ndarray, float]:
    """
    Filter beat outliers using beat-to-median distance.
    Returns:
      kept_beats, keep_mask, threshold
    """
    ...
    if mad < 1e-12:
        # Fall back to standard deviation-like rule when MAD collapses.
...
def build_dataset(
    ...
    for signal_index in tqdm(range(total_records), desc="Building RF dataset"):
        try:
            scp_codes = Y.scp_codes.iloc[signal_index]
            label = get_binary_label_lvh_norm(scp_codes, threshold=TARGET_THRESHOLD)
            if label is None:
                skipped.append((signal_index, "not LVH or pure NORM"))
                continue
            ...
            metadata_rows.append(
                {
                    ...
                    "lvh_score": scp_codes.get(POSITIVE_LABEL, 0),
...
def main():
    parser = argparse.ArgumentParser(description="Random Forest for LVH vs NORM with fixed64 + QC")
    ...
    global PLOT_OUT_DIR
    PLOT_OUT_DIR = out_dir  # used by build_dataset plot saver

    dataset_files = [out_dir / "X_3d.npy", out_dir / "X_flat.npy", out_dir / "y.npy", out_dir / "metadata.csv"]

    print("=" * 80)
    print("Random Forest for LVH vs NORM classification (fixed64 + QC)")
    print("=" * 80)
    print(f"Positive class     : {POSITIVE_LABEL} >= {TARGET_THRESHOLD}")
    ...
    print(f"LVH samples         : {int((y == 1).sum())}")
